...
namespace Policy.Infrastructure
{
	public class PolicyImplementation : IPolicyRepository
	{
		...
		{
			return await _policies.Find(p => p.Id == policyId).FirstOrDefaultAsync();
		}
		...
		{
			await _policies.ReplaceOneAsync(p => p.Id == policy.Id, policy);
		}

		public async Task DeletePolicyAsync(string policyId)
		{
			await _policies.DeleteOneAsync(p => p.Id == policyId);
		}
	}
}
